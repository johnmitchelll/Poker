...
function getWinner(hand1Input, hand2Input, doNotDisplay){
	...
	if(rank1 != rank2){
		if(rank1 < rank2){
			if(!doNotDisplay){
				command = getCommandFromWin("Human", best1.message);
			}
		...
		if(!doNotDisplay){
			command = getCommandFromWin("A.I.", best2.message);
		}
	...
	for (var i = 0; i < best1.cards.length; i++) {
		if(best1.cards[i] > best2.cards[i]){
			if(!doNotDisplay){
				command = getCommandFromWin("Human", best1.message, getCardNameFromNumber(best1.cards[i]), i);
			}
		...
		if(best1.cards[i] < best2.cards[i]){
			if(!doNotDisplay){
				command = getCommandFromWin("A.I.", best2.message, getCardNameFromNumber(best2.cards[i]), i);
			}
